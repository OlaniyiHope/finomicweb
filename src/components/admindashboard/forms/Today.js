...
const Today = () => {
  ...
  useEffect(() => {
    const fetchChatHistory = async () => {
      try {
        const response = await axios.get(`${apiUrl}/api/ai/fin-inquiry/today`, {
          headers: {
        ...
        if (response.data && response.data.data) {
          ...
          const flatMessages = response.data.data.flat();

          const chatMessages = flatMessages.map((msg) => ({
            sender: msg.sender || "user", // fallback if sender is undefined
            text: msg.user_message,
            aiResponse: msg.ai_response,
          }));

          setMessages(chatMessages.reverse()); // Optional: latest message last
        }
      } catch (err) {
